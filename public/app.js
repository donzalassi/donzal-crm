...
async function syncData() {
  ...
  try {
    const response = await fetch(`${API_BASE}/api/data/load`, {
      method: 'POST',
...
async function persistData(payload) {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/data/save`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
function filterCustomers(type) {
  ...
  if (type === 'vip') {
    ...
    const vipTmp = dbConfig.vipTemplate || `[${dbConfig.storeName}] 우수 고객님께!\n5만원 상당의 고기 추가 쿠폰을 드립니다.\n다음 방문 시 제시해주세요.`;
    smsMsg.value = vipTmp;
    ...
    const dormantTmp = dbConfig.dormantTemplate || `[${dbConfig.storeName}] 보고 싶었어요!\n재방문 시 불고기 2인분을 서비스로 대접해 드릴게요!\n이번 주말 꼭 들러주세요.`;
    smsMsg.value = dormantTmp;
    smsCnt.textContent = `대상: 총 ${result.length}명`;
...
async function sendTargetSMS(type) {
  ...
  let text = '';
  if (type === 'vip') {
    const vEdit = document.getElementById('target-msg-vip');
    text = (vEdit && vEdit.value) ? vEdit.value : (dbConfig.vipTemplate || '');
  } else if (type === 'dormant') {
    const dEdit = document.getElementById('target-msg-dormant');
    text = (dEdit && dEdit.value) ? dEdit.value : (dbConfig.dormantTemplate || '');
  }

  // 고객 관리 탭에서 개별 발송 시 처리
  if (!text) {
    const mainSms = document.getElementById('sms-message');
    if (mainSms && mainSms.value) text = mainSms.value;
  }
...
async function saveConfig() {
  ...
  try {
    await fetch(`${API_BASE}/api/admin/update-profile`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ 
